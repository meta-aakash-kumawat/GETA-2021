...
public class ImplementationUsingMatrix implements UndirectedWeightedGraph 
{
	 private int[][] graph;
	    ...
	    public ImplementationUsingMatrix(int[][] graph)  
	    {
	        if (graph == null)
	            System.out.println("Graph can't be Null");
	    ...
	    public List<Integer> reachable(int node) 
	    {
	        if (node >= graph.length || node < 0)
	            System.out.println("Given Node not Present in Graph");
	    ...
	    public int shortestPath(int node1, int node2) 
	    {
	        if (node1 >= graph.length || node1 < 0 || node2 >= graph.length || node2 < 0)
	        	System.out.println("Given Nodes not Present in Graph");
